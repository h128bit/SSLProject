...
device: str|None=None, T: float=0.1)
```
Base abstract class
...
theta: float=0.98, device: str|None=None, T: float=0.1)
```
...
temperature: float=0.1, alpha: float=1, sigma: float=0.5, kappa: float=0.5, eta: float=5.0, T: float=0.1, symmetric: bool=True)
```
...
By default: None
- temperature: temperature for InfoNCE loss. By default: 0.1
- alpha: coefficient for cross entropy loss. By default: 0.1
- sigma: coefficient for nnclr loss. By default: 0.5
- kappa: coefficient for centroid loss. By default: 0.5
- eta: coefficient for red loss. By default: 5.0
- T: temperature for cross entropy loss. By default: 0.1
- symmetric: if set to True when both view of augmented image one by one pass to teacher and student model. By default: True
...
Compute as <code>base_loss = -(softmax(teacher_out / T) * log_softmax(student_out / T)).sum(dim=1).mean() * (T ** 2)</code>
